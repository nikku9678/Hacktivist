      ...
      lg:grid-cols-2'>
        ...
        <div className='mb-5 flex items-center gap-3'>
          ...
          <div className='relative w-[160px] h-[50px]'>
            ...
            id='customFile'
            onChange={handleInputChange}
            accept='.jpg,.png' className='absolute top-0 left-0 w-full
            h-full opacity-0 cursor-pointer' />
            <label htmlFor="customFile"
            className='absolute top-0 left-0
            w-full h-full flex items-center justify-center px-[0.75rem]
            py-[0.375rem] text-[15px] leading-6 overflow-hidden
            bg-[#0066ff46] text-headingColor
            font-semibold
            rounded-lg truncate cursor-pointer'>Upload Photo</label>
          </div>
        </div>
        <div className='mt-7'>
          <button disabled={loading && true} type='submit' className='w-full bg-primaryColor
          text-white text-[18px] leading-[30px] px-4 py-3
          rounded-lg'>{loading ? (<HashLoader size={35} color="#ffffff"/>) :("Sign Up")}</button>
        </div>
